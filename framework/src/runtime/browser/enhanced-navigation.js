...
(function () {
  ...
  function renderCheckoutPage() {
    ...
    contentRoot.innerHTML = `
      <section class="storefront-cart-page storefront-checkout-page">
        ...
        <div class="storefront-container storefront-checkout-notification-wrap">
          <div class="storefront-checkout-notification" data-checkout-notification hidden>
            <div class="storefront-checkout-notification__copy">
              <strong data-checkout-notification-title></strong>
              <p data-checkout-status></p>
              <small data-checkout-notification-note></small>
            </div>
            <div class="storefront-checkout-notification__actions" data-checkout-notification-actions></div>
          </div>
        </div>
        <div class="storefront-container storefront-cart-page__content">
        ${items.length === 0 ? `
  ...
  async function handleCheckoutSubmit(form) {
    ...
    try {
      await synchronizeRuntimeCart(order.items);
      ...
      if (!response.ok || payload.error) {
        var checkoutError = new Error(checkoutErrorMessage(payload, "Không thể gửi đơn hàng lúc này."));
        checkoutError.payload = payload;
        throw checkoutError;
      }
      ...
      renderCheckoutNotification(error.payload, error.message || "Runtime/API chưa sẵn sàng.");
      restoreSubmit();
      return;
    }
  ...
  function renderCheckoutNotification(payload, fallback) {
    var notification = document.querySelector("[data-checkout-notification]");
    var title = notification && notification.querySelector("[data-checkout-notification-title]");
    var message = notification && notification.querySelector("[data-checkout-status]");
    var note = notification && notification.querySelector("[data-checkout-notification-note]");
    var actions = notification && notification.querySelector("[data-checkout-notification-actions]");
    var diagnostic = payload && payload.diagnostics && payload.diagnostics.errors && payload.diagnostics.errors[0];
    var presentation = diagnostic && diagnostic.presentation;
    if (!notification || !title || !message || !actions) return;

    title.textContent = presentation?.title || "Không thể gửi đơn hàng";
    message.textContent = presentation?.message || fallback;
    note.textContent = "Đơn hàng chưa được gửi. Giỏ hàng và thông tin của anh vẫn được giữ nguyên.";
    actions.innerHTML = checkoutNotificationActions(presentation?.actionId);
    notification.hidden = false;
    notification.dataset.state = "error";
    notification.scrollIntoView({ behavior: "smooth", block: "center" });
  }

  function checkoutNotificationActions(actionId) {
    if (actionId === "edit-coupon") {
      return '<button type="button" class="storefront-button" data-checkout-notification-action="remove-coupon">Xóa mã ưu đãi</button>'
        + '<button type="button" class="storefront-button-secondary" data-checkout-notification-action="replace-coupon">Nhập mã khác</button>';
    }

    if (actionId === "edit-cart") {
      return '<a class="storefront-button" href="/cart">Kiểm tra giỏ hàng</a>';
    }

    if (actionId === "retry") {
      return '<button type="button" class="storefront-button" data-checkout-notification-action="retry-checkout">Thử lại</button>';
    }

    return "";
  }

  function focusCheckoutCoupon() {
    window.setTimeout(function () {
      var input = document.querySelector("[data-coupon-form] input[name='coupon']");
      if (input) input.focus();
    }, 0);
  }

  async function synchronizeRuntimeCart(items) {
    var clear = await fetch("/api/cart", { credentials: "same-origin", method: "DELETE" });
    if (!clear.ok) throw new Error(runtimeErrorMessage(await clear.json().catch(function () { return {}; }), "Không thể chuẩn bị giỏ hàng trên Runtime."));

    for (var item of items) {
      var response = await fetch("/api/cart/items", {
        body: JSON.stringify({
          productId: item.productId,
          quantity: cartItemQuantity(item),
          variationId: item.variant && item.variant.id || null
        }),
        credentials: "same-origin",
        headers: { "content-type": "application/json" },
        method: "POST"
      });
      if (!response.ok) throw new Error(runtimeErrorMessage(await response.json().catch(function () { return {}; }), "Không thể đồng bộ sản phẩm vào Runtime."));
    }
  }

  function clearRuntimeCart() {
    fetch("/api/cart", { credentials: "same-origin", method: "DELETE" }).catch(function () {});
  }
  ...
  function runtimeErrorMessage(payload, fallback) {
    ...
    return payload && (payload.error || diagnostic && diagnostic.message) || fallback;
  }

  function checkoutErrorMessage(payload, fallback) {
    var diagnostic = payload && payload.diagnostics && payload.diagnostics.errors && payload.diagnostics.errors[0];
    var presentation = diagnostic && diagnostic.presentation;
    if (presentation) {
      return [presentation.title, presentation.message, presentation.action].filter(Boolean).join(". ");
    }
    var providerCode = String(diagnostic && diagnostic.detail || "").toLowerCase();
    var message = String(diagnostic && diagnostic.message || payload && payload.error || fallback);

    if (providerCode.includes("coupon") || /coupon|mã ưu đãi/i.test(message)) {
      return "Mã ưu đãi không hợp lệ, đã hết hạn hoặc không áp dụng cho đơn hàng này. Hãy xóa mã ưu đãi rồi thử lại.";
    }

    if (providerCode.includes("product") || providerCode.includes("variation")) {
      return "Sản phẩm hoặc cấu hình đã chọn không còn hợp lệ. Hãy quay lại sản phẩm, chọn lại cấu hình rồi đặt hàng lại.";
    }

    if (providerCode.includes("payment")) {
      return "Hình thức thanh toán hiện chưa khả dụng. Hãy chọn phương thức khác hoặc liên hệ Tín Sinh Phát để được hỗ trợ.";
    }
  ...
  document.addEventListener("click", function (event) {
    ...
    var notificationAction = event.target.closest && event.target.closest("[data-checkout-notification-action]");

    if (notificationAction) {
      event.preventDefault();
      if (notificationAction.dataset.checkoutNotificationAction === "remove-coupon") {
        clearCoupon();
        return;
      }
      if (notificationAction.dataset.checkoutNotificationAction === "replace-coupon") {
        clearCoupon();
        focusCheckoutCoupon();
        return;
      }
      if (notificationAction.dataset.checkoutNotificationAction === "retry-checkout") {
