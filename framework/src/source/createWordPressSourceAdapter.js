...
import { createWordPressContentContract } from "./wordpressContentContract.js";
...
export default function createWordPressSourceAdapter(options = {}) {
  ...
  return Object.freeze({
    ...
    async getContentContract() {
      ...
      return createWordPressContentContract({
        authors: collections.authors ?? [],
        contents,
        terms: collections.terms ?? []
      });
    },
    async getCommerceContract() {
      if (!endpoint) throw new Error("WordPress endpoint is not initialized.");
      if (!woocommerceEnabled) return null;
      return createContentAdapter().getCommerceContract({ siteId });
    },
    ...
    async initialize(input = {}) { endpoint = normalizeEndpoint(input.endpoint || endpoint); siteId = input.siteId || siteId; return endpoint ? { ok: true } : diagnostics("wordpress.source.endpoint.required", "WordPress endpoint is required."); },
