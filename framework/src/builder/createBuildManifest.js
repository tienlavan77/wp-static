export default function createBuildManifest(sitePlan, buildResult, options = {}) {
  return {
    ...
    data: {
      contentManifestPath: buildResult.contentStore?.manifestPath ?? null,
      contentFilesWritten: buildResult.contentStore?.filesWritten ?? 0,
      manifestPath: buildResult.routeData?.manifestPath ?? null,
      routesWritten: buildResult.routeData?.routesWritten ?? 0
    },
    media: {
      count: buildResult.mediaManifest?.count ?? 0,
      manifestPath: buildResult.mediaManifest?.manifestPath ?? null
    },
    routing: {
      manifestPath: buildResult.routeManifest?.manifestPath ?? null,
      redirects: buildResult.routeManifest?.manifest?.redirects?.length ?? 0
    },
    templates: {
