...
import createSearchService from "../../search/createSearchService.js";
...
export default async function writeSearchIndex(sitePlan, options = {}) {
  ...
  const items = (sitePlan.routes ?? [])
    ...
    .filter(Boolean);
  const service = createSearchService({ siteId: options.siteId ?? options.site?.siteId ?? options.site?.id ?? null });
  const payload = {
