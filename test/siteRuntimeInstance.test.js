...
test("Site Runtime Instance wires real services into one Router composition", async () => {
  ...
  try {
    ...
    const instance = createSiteRuntimeInstance({
      ...
      repository,
      themeRenderer: { render: () => ({ diagnostics: { errors: [], warnings: [] }, ok: true, pages: [] }) },
    ...
    assert.equal((await instance.router.handle({ host: "example.test", method: "GET", path: "/" })).body.route, "installer");
